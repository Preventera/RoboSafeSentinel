...
class RoboSafeSentinel:
    ...
    async def _init_sensors(self) -> None:
        ...
        if self.simulate:
            ...
            await self.plc_driver.start_cyclic_read(100)
            await self.robot_driver.start_cyclic_read(50)
            await self.scanner_driver.start_cyclic_read(100)
            await self.fumes_driver.start_cyclic_read(500)
    ...
    def _collect_all_sensors(self) -> dict:
        ...
        if self.robot_driver and self.robot_driver.is_connected:
            ...
            if status:
                ...
                signals["fanuc_mode"] = status.mode.name if hasattr(status.mode, 'name') else str(status.mode)
        ...
        if self.scanner_driver and self.scanner_driver.is_connected:
            ...
            if measurement:
                ...
                signals["scanner_zone_status"] = measurement.active_zone
        ...
        if self.fumes_driver and self.fumes_driver.is_connected:
            ...
            if measurement:
                ...
                signals["fumes_alert_level"] = measurement.alert_level.value if hasattr(measurement.alert_level, 'value') else measurement.alert_level
        # Vision
        if self.vision_driver and self.vision_driver.is_connected:
            ...
            if result:
                ...
                signals["vision_min_distance"] = (
                    result.min_distance_mm if result.min_distance_mm != float("inf") else 10000
                )
    ...
    def _setup_safety_rules(self) -> None:
        ...
        self.rule_engine.register_rule(
            Rule(
                ...
                description="Distance critique détectée",
            )
        )
        ...
        self.rule_engine.register_rule(
            Rule(
                ...
                actions=[RuleAction(action_type="SLOW_25", message="Distance < 800mm")],
                description="Distance warning - Ralentissement 25%",
            )
        )
        ...
        self.rule_engine.register_rule(
            Rule(
                ...
                actions=[RuleAction(action_type="SLOW_50", message="Distance < 1200mm")],
                description="Distance monitoring - Vigilance",
            )
        )
        ...
        self.rule_engine.register_rule(
            Rule(
                ...
                actions=[RuleAction(action_type="ESTOP", message="Intrusion zone danger")],
                description="Intrusion zone danger détectée par vision",
            )
        )
        ...
        self.rule_engine.register_rule(
            Rule(
                ...
                name="EPI manquant",
                priority=RulePriority.P2_MEDIUM,
                condition=lambda ctx: not ctx.get("vision_ppe_ok", True)
                and ctx.get("vision_person_count", 0) > 0,
                actions=[RuleAction(action_type="ALERT", message="EPI manquant détecté")],
                description="EPI manquant détecté",
            )
        )
        ...
        self.rule_engine.register_rule(
            Rule(
                ...
                actions=[RuleAction(action_type="ESTOP", message="Arrêt d'urgence physique")],
                description="Arrêt d'urgence physique activé",
            )
        )
    ...
    async def _main_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                triggered_rules = await self.rule_engine.evaluate_all()
